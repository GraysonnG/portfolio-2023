<script lang="ts">
	...
	onMount(() => {
		...
		setInterval(() => {
			if (svgs.length > 0) {
				const shape: HTMLElement | undefined = svgs[Math.floor(Math.random() * svgs.length)] as
	...
	function handleMouseOverSVG(event: MouseEvent) {
		...
		svgs.forEach((svg) => {
			...
			const xWithinHitArea =
				rect.left + (rect.width * (hitArea / 100)) / 2 <= mouseX &&
				mouseX <= rect.right - (rect.width * (hitArea / 100)) / 2;
			const yWithinHitArea =
				rect.top + (rect.height * (hitArea / 100)) / 2 <= mouseY &&
				mouseY <= rect.bottom - (rect.height * (hitArea / 100)) / 2;
...
<div class="overlay" bind:clientWidth>
	{#each stars as star, index}
		...
		>
			<circle
				...
				cy="5"
				r="3"
				style="fill: transparent; stroke: var(--color-decoration); stroke-width: 1.5;"
			/>
		</svg>
	{/each}
